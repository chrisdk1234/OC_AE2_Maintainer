...
local function cpuReport()
    ...
    for _, cpu in pairs(info.list) do
        ...
        local line = string.format("   [%d] %-20s %8d bytes  %2d co-proc  %s",
            ...
            tonumber(cpu.coprocessors) or 0,
            cpu.busy and "BUSY" or "idle")
        colorPrint(cpu.busy and colors.yellow or colors.green, line)
    end

    colorPrint(colors.cyan, string.format("   → %d idle / %d total, largest idle CPU: %d bytes",
        info.free, info.total, info.maxFreeStorage))
    colorPrint(colors.cyan, string.format("   → maxConcurrentCrafts = %d, so at most %d job(s) can ever run",
        cfg.maxConcurrentCrafts, math.min(cfg.maxConcurrentCrafts > 0 and cfg.maxConcurrentCrafts or info.total, info.total)))

    if info.free == 0 then
        colorPrint(colors.red, "   ⚠ All CPUs are busy right now - requests fail until one frees up")
    end
